...
import playground.part2eventsourcing.PersistentActors.InvoiceRecorded
...
object MultiplePersists extends App {
  ...
  /*
     ...
     - an invoice record for peronsal logs or some auditing authority
   */
    ...
    override def receiveCommand: Receive = {
      case Invoice(recipient, date, amount) => {
        persist(TaxRecord(taxId, latestTaxRecordId, date, amount / 3)) { record =>
          ...
          persist("I heareby declare this tax record to be true and complete.") { declaration =>
        ...
        persist(InvoiceRecord(latestInvoiceRecordId, recipient, date, amount)) { invoiceRecord =>
          ...
          persist("I heareby declare this invoice record to be true and complete.") { declaration =>
            taxAuthority ! declaration
          }
        }
      }
  ...
  val system       = ActorSystem("MultuplePersistsDemo")
  val taxAuthority = system.actorOf(Props[TaxAuthority], "HMRC")
  val accountant   = system.actorOf(DiligentAccountant.props("UK2543_153", taxAuthority))

  accountant ! Invoice("The Sofa Company", new Date, 2000)

  /*
    The message ordering (TaxRecord -> Invoice Record) is GUARANTEED.
   */
  ...
  accountant ! Invoice("The Super Car Company", new Date, 20004302)
}
