...
//  }
  ...
  class SimplePersistenceActor extends PersistentActor with ActorLogging {
    ...
    override def receiveCommand: Receive = {
      case m =>
        persist(m) { _ => log.info(s"Persisted: $m")
        }
    }

    override def receiveRecover: Receive = {
      case e => log.info(s"Receovered: $e")
    }
  }

  val simpleActor = system.actorOf(Props[SimplePersistenceActor], "simplePersistenceActor")
  ...
  // events by tags
  val genres = Array("Pop", "rock", "hiphop", "jazz", "disco")
  ...
  rockPlaylists.runForeach { event => println(s"Found a playlist with a rock song: $event")
  }
